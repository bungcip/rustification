...
//! use rast::{braced, parse_macro_input, token, Field, Ident, Result, Token};
...
//! ```
//!
//! # The `rast::parse*` functions
//!
//! The [`rast::parse`], [`rast::parse2`], and [`rast::parse_str`] functions serve
//! as an entry point for parsing syntax tree nodes that can be parsed in an
//! obvious default way. These functions can return any syntax tree node that
//! implements the [`Parse`] trait, which includes most types in Syn.
//!
//! [`rast::parse`]: crate::parse()
//! [`rast::parse2`]: crate::parse2()
//! [`rast::parse_str`]: crate::parse_str()
//!
//! ```
//! use rast::Type;
//!
//! # fn run_parser() -> rast::Result<()> {
//! let t: Type = rast::parse_str("std::collections::HashMap<String, Value>")?;
//! #     Ok(())
//! # }
//! #
//! # run_parser().unwrap();
...
//! #
//! # use rast::punctuated::Punctuated;
//! # use rast::{PathSegment, Result, Token};
//! #
//! # fn f(tokens: proc_macro::TokenStream) -> Result<()> {
//! #
//! // Can't parse `Punctuated` without knowing whether trailing punctuation
//! // should be allowed in this context.
//! let path: Punctuated<PathSegment, Token![::]> = rast::parse(tokens)?;
//! #
//! #     Ok(())
//! # }
//! ```
...
//! #
//! use proc_macro::TokenStream;
//! use rast::parse::Parser;
//! use rast::punctuated::Punctuated;
//! use rast::{Attribute, Expr, PathSegment, Result, Token};
//!
...
/// - One of [the `rast::parse*` functions][syn-parse]; or
...
impl<'a> ParseBuffer<'a> {
    ...
    /// - `input.peek(Token![==])`
    /// - `input.peek(rast::Ident)`&emsp;*(does not accept keywords)*
    /// - `input.peek(rast::Ident::peek_any)`
    /// - `input.peek(Lifetime)`
    ...
    ///
    /// ```
    /// use rast::{braced, token, Generics, Ident, Result, Token, TypeParamBound};
    /// use rast::parse::{Parse, ParseStream};
    /// use rast::punctuated::Punctuated;
    ///
    ...
    ///
    /// ```
    /// use rast::{Ident, ItemUnion, Macro, Result, Token};
    /// use rast::parse::{Parse, ParseStream};
    ///
    ...
    /// # use quote::quote;
    /// #
    /// use rast::{parenthesized, token, Ident, Result, Token, Type};
    /// use rast::parse::{Parse, ParseStream};
    /// use rast::punctuated::Punctuated;
    ///
    ...
    /// #     struct S(A, B);
    /// # };
    /// # rast::parse2::<TupleStruct>(input).unwrap();
    /// ```
    ///
    /// # See also
    ///
    ...
    ///
    /// ```
    /// use rast::{custom_keyword, Expr, Result, Token};
    /// use rast::parse::{Parse, ParseStream};
    /// use rast::punctuated::Punctuated;
    ///
    /// mod kw {
    ///     rast::custom_keyword!(fin);
    /// }
    ///
    ...
    ///
    /// ```
    /// use rast::{braced, token, Ident, Item, Result, Token};
    /// use rast::parse::{Parse, ParseStream};
    ///
    ...
    ///
    /// ```
    /// use rast::{ConstParam, Ident, Lifetime, LifetimeParam, Result, Token, TypeParam};
    /// use rast::parse::{Parse, ParseStream};
    ///
    ...
    ///
    /// ```
    /// use rast::{parenthesized, token, Ident, Path, Result, Token};
    ...
    /// use rast::buffer::Cursor;
    /// use rast::parse::{ParseStream, Result};
    ///
    ...
    /// fn main() {
    ///     use quote::quote;
    ///     use rast::parse::{Parse, Parser};
    ///     use rast::Token;
    ///
    ///     // Parse rast::Type as a TokenStream, surrounded by angle brackets.
    ///     fn example(input: ParseStream) -> Result<TokenStream> {
    ///         let _langle: Token![<] = input.parse()?;
    ///         let ty = recognize_token_stream(rast::Type::parse)(input)?;
...
/// #
/// use proc_macro::TokenStream;
/// use rast::parse_macro_input;
/// use rast::parse::Nothing;
///
/// # const IGNORE: &str = stringify! {
