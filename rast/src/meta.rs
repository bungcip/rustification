...
/// use rast::{parse_macro_input, LitStr, Path};
///
/// # const IGNORE: &str = stringify! {
...
///     let tea_parser = rast::meta::parser(|meta| {
...
/// ```
///
/// The `rast::meta` library will take care of dealing with the commas including
...
/// #
/// use proc_macro::TokenStream;
/// use rast::meta::ParseNestedMeta;
/// use rast::parse::{Parser, Result};
/// use rast::{parse_macro_input, LitStr, Path};
///
/// # const IGNORE: &str = stringify! {
/// #[proc_macro_attribute]
...
///     let tea_parser = rast::meta::parser(|meta| attrs.parse(meta));
///     parse_macro_input!(args with tea_parser);
///
///     /* ... */
/// #   TokenStream::new()
...
///
/// - [`rast::meta::parser`] if you are implementing a `proc_macro_attribute`
...
///
/// [`Attribute::parse_nested_meta`]: crate::Attribute::parse_nested_meta
/// [`rast::meta::parser`]: crate::meta::parser
#[non_exhaustive]
...
impl<'a> ParseNestedMeta<'a> {
    ...
    ///
    /// ```
    /// use rast::{parse_quote, Attribute, LitStr};
    ///
    ...
    ///
    /// ```
    /// use rast::{parse_quote, Attribute};
    ///
    /// let attr: Attribute = parse_quote! {
    ///     #[tea(with(sugar, milk))]
    /// };
    ...
    ///
    /// ```
    /// use rast::{parenthesized, parse_quote, Attribute, LitInt};
    ///
    /// let attr: Attribute = parse_quote! {
    ///     #[repr(align(32))]
    /// };
    ...
    /// # use rast::Attribute;
    /// #
    /// # fn example(attr: &Attribute) -> rast::Result<()> {
    /// attr.parse_nested_meta(|meta| {
    ///     if meta.path.is_ident("kind") {
    ///         // ...
    ///         Ok(())
    ...
    /// `rast::Error::new_spanned(&meta.path, "message...")`.
    ///
    /// ```console
    ...
    /// ```
    ///
    /// Often you may want to use `rast::Error::new_spanned` even in this
    /// situation. In the above code, that would be:
    ///
    /// ```
    /// # use rast::{Error, Expr};
    /// #
    /// # fn example(expr: Expr) -> rast::Result<()> {
    ///     match expr {
    ///         Expr::Lit(expr) => /* ... */
    /// #           unimplemented!(),
    ///         Expr::Path(expr) => /* ... */
