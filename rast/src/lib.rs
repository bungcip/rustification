...
mod gen {
    ...
    ///
    /// ```
    /// # use rast::{Attribute, BinOp, Expr, ExprBinary};
    /// #
    /// pub trait Fold {
    ///     /* ... */
    ///
    ...
    ///
    /// use quote::quote;
    /// use rast::fold::{fold_expr, Fold};
    /// use rast::{token, Expr, ExprParen};
    ///
    ...
    ///     let expr: Expr = rast::parse2(code).unwrap();
    ...
    ///
    /// use quote::quote;
    /// use rast::visit::{self, Visit};
    /// use rast::{File, ItemFn};
    ///
    /// struct FnVisitor;
    ///
    ...
    ///     };
    ///
    ///     let syntax_tree: File = rast::parse2(code).unwrap();
    ...
    ///
    /// use quote::quote;
    /// use rast::visit_mut::{self, VisitMut};
    /// use rast::{parse_quote, Expr, File, Lit, LitInt};
    ///
    ...
    ///                     let unsuffixed: LitInt = rast::parse_str(digits).unwrap();
    ...
    ///     };
    ///
    ///     let mut syntax_tree: File = rast::parse2(code).unwrap();
