...
impl Macro {
    ...
    /// tree.
    ///
    /// This is equivalent to `rast::parse2::<T>(mac.tokens)` except that it
    /// produces a more useful span when `tokens` is empty.
    ///
    /// # Example
    ///
    /// ```
    /// use rast::{parse_quote, Expr, ExprLit, Ident, Lit, LitStr, Macro, Token};
    /// use rast::ext::IdentExt;
    /// use rast::parse::{Error, Parse, ParseStream, Result};
