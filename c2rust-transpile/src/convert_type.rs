    ...
    */
    ...
    pub fn convert_pointer(
        ...
        match ctxt.resolve_type(qtype.ctype).kind {
            ...
            CTypeKind::Void => Ok(mk()
                .set_mutbl(mutbl)
                .ptr_ty(mk().path_ty(vec!["ffi", "c_void"]))),
    ...
    pub fn convert(
        ...
        match ctxt.index(ctype).kind {
            ...
            CTypeKind::Int => Ok(mk().path_ty(mk().path(vec!["ffi", "c_int"]))),
            CTypeKind::Long => Ok(mk().path_ty(mk().path(vec!["ffi", "c_long"]))),
            CTypeKind::LongLong => Ok(mk().path_ty(mk().path(vec!["ffi", "c_longlong"]))),
            CTypeKind::UShort => Ok(mk().path_ty(mk().path(vec!["ffi", "c_ushort"]))),
            CTypeKind::UInt => Ok(mk().path_ty(mk().path(vec!["ffi", "c_uint"]))),
            CTypeKind::ULong => Ok(mk().path_ty(mk().path(vec!["ffi", "c_ulong"]))),
            CTypeKind::ULongLong => Ok(mk().path_ty(mk().path(vec!["ffi", "c_ulonglong"]))),
            CTypeKind::SChar => Ok(mk().path_ty(mk().path(vec!["ffi", "c_schar"]))),
            CTypeKind::UChar => Ok(mk().path_ty(mk().path(vec!["ffi", "c_uchar"]))),
            CTypeKind::Char => Ok(mk().path_ty(mk().path(vec!["ffi", "c_char"]))),
            CTypeKind::Double => Ok(mk().path_ty(mk().path(vec!["ffi", "c_double"]))),
            CTypeKind::LongDouble | CTypeKind::Float128 => {
                Ok(mk().path_ty(mk().path(vec!["f128", "f128"])))
            }
            CTypeKind::Float => Ok(mk().path_ty(mk().path(vec!["ffi", "c_float"]))),
