...
impl Printer {
    ...
    fn item_verbatim(&mut self, tokens: &TokenStream) {
        ...
        let item: ItemVerbatim = match rast::parse2(tokens.clone()) {
            Ok(item) => item,
    ...
    fn foreign_item_verbatim(&mut self, tokens: &TokenStream) {
        use rast::parse::{Parse, ParseStream, Result};
        use rast::{Attribute, Token, Visibility};
        ...
        let foreign_item: ForeignItemVerbatim = match rast::parse2(tokens.clone()) {
    ...
    fn trait_item_verbatim(&mut self, tokens: &TokenStream) {
        ...
        let impl_item: TraitItemVerbatim = match rast::parse2(tokens.clone()) {
